# User Metrics & Athlete Profile

> This file is the single source of truth for the athlete's physical profile, physiological benchmarks, biomechanics data, personal bests, and hardware. The running coach reads this file before every coaching decision.

---

## 1. Physical Profile

| Parameter | Value | Notes |
| :--- | :--- | :--- |
| **Age** | 26 | As of 2026 |
| **Height** | 187 cm | Baseline physical stat |
| **Current Weight** | 93–95 kg | Heavy muscle mass from 5-day lifting routine |
| **Target Weight** | 85 kg | Optimal for sub-20:00 5K running economy |
| **Location** | Bengaluru, India | Training environment: hot & humid conditions |
| **Athlete Type** | Heavy / Hybrid | Significant muscle mass — highly developed quads, hamstrings, glutes |

---

## 2. Cardiovascular & Physiological Metrics

| Parameter | Value | Method / Notes |
| :--- | :--- | :--- |
| **Tested Max HR** | 206 BPM | Recorded July 1, 2026 (6x400m intervals) |
| **Resting HR (RHR)** | 65 BPM | Karvonen baseline |
| **HR Reserve (HRR)** | 141 BPM | Max HR − RHR = 206 − 65 |
| **Lactate Threshold HR** | ~170 BPM | Estimated from performance data — crossing this in early race laps causes catastrophic positive splits |
| **Lactate Threshold Pace** | ~5:35–5:40 /km | Current ability (July 2026). Target for sub-20: 4:10–4:15/km |
| **VO2 Max (estimated)** | Moderate-High | High pain tolerance at 198 BPM for 19+ min suggests strong cardiovascular engine; limiter is structural/weight |

---

## 3. Karvonen Heart Rate Zones

Calculated: **Max HR 206 BPM | RHR 65 BPM | HRR 141 BPM**

| Zone | Name | BPM Range | % HRR | RPE | Training Purpose |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Zone 1** | Active Recovery | 136 – 148 | 50–60% | 1–2 | Flush runs, warm-ups, cool-downs |
| **Zone 2** | Easy / Aerobic Base | 149 – 162 | 60–70% | 3–4 | Aerobic base building. Must be conversational. Cap for easy days: **≤ 150 BPM** |
| **Zone 3** | Tempo / Threshold Entry | 163 – 177 | 70–80% | 5–6 | Sustainable steady effort |
| **Zone 4** | Lactate Threshold / Hard | 178 – 191 | 80–90% | 7–8 | Comfortably hard. Primary interval work zone |
| **Zone 5** | VO2 Max / Redline | 192 – 206 | 90–100% | 9–10 | Short intervals only. Absolute maximum stimulus |

> ⚠️ **Critical Marker**: Lactate Threshold sits at ~170 BPM. Crossing this before minute 10 in a 5K time trial guarantees a positive split collapse.

---

## 4. Biomechanics Profile

| Metric | Current Default (Flawed) | Target (Optimal) | Consequence of Default |
| :--- | :--- | :--- | :--- |
| **Cadence** | 154–156 SPM | **Speed-Dependent:**<br>• Easy/LSD: **150–153 SPM**<br>• Tempo/TT/Intervals: **164+ SPM** | Overstriding at fast pace, knee flexion strain/heavy impact |
| **Stride Length** | 1.15m | **Speed-Dependent:**<br>• Easy/LSD: **0.72–0.75m**<br>• Tempo/TT/Intervals: **1.03m** | Overstriding at fast pace, knee flexion strain/heavy impact |
| **Foot Strike** | In front of center of gravity | Under center of gravity | Creates braking forces instead of propulsive forces |
| **Primary Load** | Achilles / calves | Glutes / quads | Fragile connective tissue absorbing forces it shouldn't |
| **Vertical Oscillation** | High (bouncing) | Minimal | Energy waste + impact multiplication at high body weight |

### Biomechanics Golden Ratio
> **Established: July 25, 2026** from 5K Time Trial lap data.
> **164 SPM at 1.03m stride** = optimal formula for fast running (≤6:30/km).
> **150–153 SPM at 0.72–0.75m stride** = sweet spot for easy running (9:00–9:20/km) to prevent knee strain and sluggish impact for 187cm runner.

### Death Shuffle Immunity Protocol
When fatigue hits late in a race or workout:
- ❌ Natural response: Cadence drops to 154 SPM, stride shrinks to 0.95m, pace collapses.
- ✅ Required response: **Let stride shrink (0.95–1.00m is fine), but force cadence to stay at 164 SPM.**
- Rationale: Cadence maintenance preserves momentum without requiring power from fatigued glycogen stores.

---

## 5. Personal Bests & Performance Benchmarks

| Distance / Effort | Time / Pace | Avg HR | Date | Notes |
| :--- | :--- | :--- | :--- | :--- |
| **5K Time Trial** | 30:45 (6:09/km) | 186 BPM | July 25, 2026 | Month 2 Baseline. Positive split collapse. Adrenaline pacing error in Lap 1. |
| **5K Unofficial** | ~29:30 est. | — | — | With perfect negative split execution, current engine capable of ~29:30 |
| **3.5K Tempo** | 19:32 (5:35/km) | 185 BPM | July 17, 2026 | Redline effort. True max threshold test. |
| **400m Interval (fastest)** | 1:51 (4:36/km) | Peak 182 | July 8, 2026 | Set during 6x400m session |
| **12.4km LSD** | 1:53:18 (9:08/km) | 147 BPM | July 12, 2026 | Zone 2 masterclass but structural overreach — 55% over 8km target |
| **VDOT Estimate** | ~38–40 | — | July 2026 | Based on interval pace and HR data. Target for sub-20: ~54 |

---

## 6. Injury & Structural Flags

| Issue | Status | Notes |
| :--- | :--- | :--- |
| **Achilles Tendon (bilateral)** | 🟡 Monitor | Low cadence (156 SPM) + 1.15m stride at 95 kg causes high eccentric load. Flared post July 12 overreach. |
| **Calf Overload** | 🟡 Monitor | 90+ mins at 95 kg on feet causes cumulative structural fatigue. Enforce distance caps. |
| **General Connective Tissue** | 🟡 Developing | Engine (cardiovascular) currently ahead of chassis (tendons, ligaments). Slow structural adaptation required. |

---

## 7. Nutrition & Recovery Protocol

| Parameter | Target | Notes |
| :--- | :--- | :--- |
| **Daily Protein** | 180–200g | Protect muscle mass during caloric deficit |
| **Caloric Deficit** | 300–500 kcal/day | Safe rate to reach 85 kg target by Feb 2027 |
| **Daily Hydration** | 3–4 litres | Critical for Bengaluru heat conditions |
| **Gym Training** | 5 days/week | Heavy lifting maintained alongside running program |

---

## 8. Hardware & Tracking

| Device | Model | Notes |
| :--- | :--- | :--- |
| **Smartwatch** | Amazfit T-Rex 3 Pro | Paired with Zepp App |
| **Cadence Assist** | 82 BPM single-foot anchor | Metronome target = 164 SPM total (82 x 2) |
| **Pacing Strategy** | Customized outdoor templates | Exact distance/pace alerts enforce negative split discipline |
| **Arm Mechanics** | Phone stowed during runs | Prevents asymmetric arm swing from holding device |
